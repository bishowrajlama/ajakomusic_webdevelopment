...
class Benefits extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('benefits_model','content');
...
//                    exit;
        ...
        $data['title']= "Add/Edit Benefits";
        $this->load->view('header', $data);
        $this->load->view('benefits_form');
        $this->load->view('footer');
    }
...
//code to add new tags in content
    ...
    public function send($content_id)
    {
        ...
        if(! empty($content_detail) && $content_detail['content_type'] == 'Article' && ! empty($subscribes))
        {
            ...
            $server_url = $site_settings['site_url'].'/index.php/benefits/detail/'.$content_detail['content_url'];
