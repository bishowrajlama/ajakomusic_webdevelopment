...
class Tutorials extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('tutorials_model','content');
    ...
    public function index()
    {
        ...
        $data['title']= "tutorials List";
        $this->load->view('header', $data);
        $this->load->view('tutorials_list');
...
//                    exit;
        ...
        $data['title']= "Add/Edit tutorials";
        $this->load->view('header', $data);
        $this->load->view('tutorials_form');
        $this->load->view('footer');
    }
    ...
    public function slider_content($content_id){
        if(strlen($content_id) <= 0){ redirect('tutorials'); }
        ...
        if($this->input->post()){
            ...
            if(strlen($_FILES['image']['name']) > 0){
                ...
                if(in_array($extension,$allowed_type)){
                    ...
                    redirect('tutorials/list_photos/'.$this->input->post('content_id')); 
                }
            }else{
                if($id > 0){
...
//code to add new tags in content
    ...
    public function send($content_id)
    {
        ...
        if(! empty($content_detail) && $content_detail['content_type'] == 'Article' && ! empty($subscribes))
        {
            ...
            $server_url = $site_settings['site_url'].'/index.php/tutorials/detail/'.$content_detail['content_url'];
...
<div style="width:580px;margin:0 auto;font:12px Arial,Helvetica,sans-serif;background:#fff">
    ...
    <table width="100%" cellspacing="0" cellpadding="5" border="0">
        ...
        <tr>
            ...
            $mail->Subject    = $site_settings['site_name']." tutorialsletter";
            ...
            if(!$mail->Send())
            {
                ...
                $this->session->set_flashdata('success', 'tutorialsLetter has been Send.');
                redirect('tutorials');
            }

        }
        else{
            $this->session->set_flashdata('error', 'Unable to Send the tutorialsletter');
            redirect('tutorials');
        }
