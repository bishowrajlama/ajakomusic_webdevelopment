...
class Oursolutions extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('oursolutions_model','content');
    ...
    public function index()
    {
        ...
        $data['title']= "Our Solutions List";
        $this->load->view('header', $data);
        $this->load->view('oursolutions_list');
...
//                    exit;
        ...
        $data['title']= "Add/Edit Our Solutions";
        $this->load->view('header', $data);
        $this->load->view('oursolutions_form');
        $this->load->view('footer');
    }
        ...
        if($this->input->post()){
            ...
            if(strlen($_FILES['image']['name']) > 0){
                ...
                if(in_array($extension,$allowed_type)){
                    ...
                    redirect('oursolutions/list_photos/'.$this->input->post('content_id')); 
                }
            }else{
                if($id > 0){
...
//code to add new tags in content
    ...
    public function send($content_id)
    {
        ...
        if(! empty($content_detail) && $content_detail['content_type'] == 'Article' && ! empty($subscribes))
        {
            ...
            $server_url = $site_settings['site_url'].'/index.php/oursolutions/detail/'.$content_detail['content_url'];
