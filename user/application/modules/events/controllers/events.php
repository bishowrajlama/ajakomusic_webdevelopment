...
class Events extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('events_model','content');
    ...
    public function form($id=0)
    {
        ...
        if($this->input->post())
        {
            ...
            $this->form_validation->set_rules('ev_date', 'ev_date');
            $this->form_validation->set_rules('ev_time', 'ev_time');
            ...
            $insert['meta_title'] = $this->input->post('meta_title');
                $insert['ev_date'] = $this->input->post('ev_date');
                $insert['ev_time'] = $this->input->post('ev_time');
...
//                    exit;
                    ...
                    if($result)
                    {
                        ...
                        $log['action_id']= "2";
                        $this->create_log($log);
                        $this->session->set_flashdata('success','Event has been updated.');
                        redirect('events');
                    }
                    else{
