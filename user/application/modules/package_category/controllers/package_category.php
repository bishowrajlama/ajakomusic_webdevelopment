    ...
    public function category($id=0)
    {
        if($this->input->post())
        {
            ...
            if($result)
            {
                ...
                $log['action_id']= "2";
                $this->category_create_log($log);
                $this->session->set_flashdata('success','Category Information has been updated.');
                redirect($this->redirect);
            }
            else{
                $this->session->set_flashdata('error','Unable to update the Category Information.');
                redirect($this->redirect);
            }
...
//            exit;
    ...
    public function sub_category()
    {
        ...
        $data['sub_categories'] = $this->crud->get_all('igc_package_category');
        $data['title']= "Category List";
        $this->load->view('header', $data);
        $this->load->view('sub_category_list');
        $this->load->view('footer');
    }
