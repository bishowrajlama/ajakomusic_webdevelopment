...
class Single extends MX_Controller{
    ...
    {
        $data['records'] = $this->single->get_content_list();

        $data['title']= "Content List";
        $this->load->view('header', $data);
        $this->load->view('single_list');
    ...
    public function form($id=0)
    {
        ...
        if($this->input->post())
        {
           ...
           // $insert['parent_id'] = $this->input->post('parent_id');
            ...
            {
                $insert['parent_id'] = $this->input->post('parent_id');
                $result = $this->single->content_insert($insert);
                //code to add tags
                ...
                $this->single->tab_insert($inserts);
                ...
                if($category == "")
                {
                    ...
                    $results = $this->single->insert_content_category($cat);
...
//                    print_r($insert);
//                    exit;
                    $result = $this->single->content_update($insert, $content_id);
                    //code to add tags
                    ...
                    $tab_data = $this->single->tab_detail($content_id);

                    if(!empty($tab_data))
                    {
                        $this->single->tab_update($inserts, $content_id);
                    }
                    else{
        ...
        $content = $this->single->get_content_detail($id);
        $tab = $this->single->tab_detail($id);

        $data['content'] = array_merge($content, $tab);

        $data['categories'] = $this->single->get_content_categories();
        $data['parent_pages'] = $this->single->get_parent_page();


        $data['scripts'] = array('themes/js/form-validator');
        $data['title']= "Add/Edit Content";
        $this->load->view('header', $data);
        $this->load->view('single_form');
        $this->load->view('footer');
    }
    ...
    public function check_name_exist($str, $content_id)
    {
        ...
        $name = $this->single->check_name_exist($string, $content_id);
    ...
    public function delete($content_id)
    {
        ...
        $result = $this->single->delete_content($content_id);
        if($result)
        {
            //code to create log
...
//code to add new tags in content
    ...
    public function add_tags($content_id, $tags)
    {
        if ($tags != '') {
            ...
            $this->single->insert_tags($tags, $content_id);
        }
    ...
    {
        $content_detail = $this->crud->get_detail($content_id, 'content_id', 'igc_content');
        $subscribes = $this->single->get_subscribers();
