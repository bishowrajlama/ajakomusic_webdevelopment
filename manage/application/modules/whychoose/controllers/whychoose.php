...
class Whychoose extends MX_Controller{
    public function __Construct()
    {
        ...
        $this->load->model('whychoose_model','content');
...
//                    exit;
        ...
        $data['title']= "Add/Edit Why Choose us";
        $this->load->view('header', $data);
        $this->load->view('whychoose_form');
        $this->load->view('footer');
    }
...
//code to add new tags in content
    ...
    public function send($content_id)
    {
        ...
        if(! empty($content_detail) && $content_detail['content_type'] == 'Article' && ! empty($subscribes))
        {
            ...
            $server_url = $site_settings['site_url'].'/index.php/whychoose/detail/'.$content_detail['content_url'];
