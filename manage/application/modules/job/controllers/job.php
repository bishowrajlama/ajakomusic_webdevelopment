...
class Job extends MX_Controller{
    ...
    {
        $data['records'] = $this->job->get_package_list();
...
//            exit;
            ...
            $insert['package_url'] = $rand = $num1.$num2;
            ...
            // $check_url =  $this->crud->check_url($job_id, 'job_id', $url, 'package_url','igc_package');
            // if(!empty($check_url))
            // {
            ...
            if($job_id == "0")
            {
                ...
                $result = $this->job->insert_package($insert);
                ...
                if($result)
                {
                    ...
                    foreach($pprice as $row =>$value) {
                        ...
                        {
                            $this->job->insert_package_price($inserts);
                        }
                    ...
                    if($result)
                    {
                        $this->session->set_flashdata('success','New Applicantion is Added Successfully');
                        redirect('job');
                    }
                    else{
                        $this->session->set_flashdata('error','Unable to Add Applicantion');
                        redirect('job');
                    }
                ...
                $detail = $this->job->get_package_detail($job_id);
                ...
                $results = $this->job->update_package($insert, $job_id);
                ...
                if($results)
                {
                    ...
                    $deletes = $this->job->delete_package_price($job_id);
                    if($deletes)
                    {
                        if(!empty($pprice)) {

                            foreach ($pprice as $row => $value) {
                                $inserts['job_id'] = $job_id;
                                $inserts['accommodation_id'] = $accommodation_id[$row];
                                $inserts['currency_id'] = $currency_id[$row];
                                $inserts['pprice'] = $value;
                                $inserts['is_front'] = ($is_front == $row) ? 'Y' : 'N';
                    ...
                    $this->session->set_flashdata('success','Applicantion is Updated Successfully');
                    redirect('job');
                }
                else{
                    $this->session->set_flashdata('error','Unable to Update Applicantion');
                    redirect('job');
                }
        ...
        $data['package'] = $this->job->get_package_detail($id);
    ...
    public function check_name_exist($str, $job_id)
    {
        $string = str_replace(" ","-",$str);
        $name = $this->job->check_name_exist($string, $job_id);
        ...
        {
            $this->form_validation->set_message('check_name_exist', 'CV name already exist.');
            return FALSE;
        }
    ...
    {
        $package = $this->job->get_package_detail($job_id);

        $result = $this->job->delete_package($job_id);
        if($result)
        {
            ...
            $this->session->set_flashdata('success','CV deleted Successfully');
            redirect('job');
        }
        else{
    ...
    public function show_front()
    {
        ...
        {
            $update['show_front']= "1";
            $this->crud->update($active_id, "job_id",$update,"igc_job");
        }
        elseif($inactive_id !="")
        {
            $update['show_front']= "0";
            $this->crud->update($inactive_id, "job_id",$update,"igc_job");
        }
        else{
    ...
    public function add_tags($job_id, $tags)
    {
        if ($tags != '') {
            $tagIn = trim(preg_replace('/\s+/', ',', $tags));
            $tags = explode(',', $tagIn);

            $this->job->insert_tags($tags, $job_id);
        }
    ...
    public function check_tour_code()
    {
        if($this->input->post())
        {
            ...
            $query = $this->db->query("select * from igc_job where tourcode = '$code' and delete_status = '0' and job_id <> '$job_id'");
    ...
    public function gallery($job_id)
    {
        ...
        if($this->input->post())
        {
            ...
            $delete = $this->crud->delete($id, 'job_id', 'igc_package_album');
            if($delete)
            {
            ...
            if($result)
            {
                $this->session->set_flashdata('success','CV Gallery has been Updated.');
                redirect('job');
            }
            else
            {
    ...
    public function destination($job_id)
    {
        ...
        if($this->input->post())
        {
            ...
            $deletes =  $this->crud->delete($id, 'job_id', 'igc_package_regions');
            $delete = $this->crud->delete($id, 'job_id', 'igc_package_destination');
            if($deletes)
            {
                if(!empty($regions)) {
            ...
            if($result)
            {
                $this->session->set_flashdata('success','CV has been Updated.');
                redirect('job');
            }
            else
            {
        ...
        $data['detail'] =  $this->crud->get_Detail($job_id,'job_id','igc_package');
