...
class News extends MX_Controller{
    ...
    public function __Construct()
    {
        ...
        $this->load->model('news_model','content');
    ...
    public function index($start_point='')
    {

            $data = array();
 $name = $this->uri->segment(1);
    $config["base_url"] = site_url('news/index');
    $config['total_rows'] =   $this->content->count_all();//here we will count all the data from the table
...
//        exit();
        ...
        $data['records']= $this->content->get_article_list($startpoint, $per_page);
// // echo "<pre>";
    ...
    public function search()
    {
        ...
        $data['job_result'] = $this->content->get_job_result($search_term);
...
//        exit();
        ...
        $data['records']= $this->content->get_notified_article_list($startpoint, $per_page);
// // echo "<pre>";
       // $data['records'] = $this->content->get_content_list();
    ...
    public function form($id=0)
    {
        ...
        if($this->input->post())
        {
// print_r($this->input->post('category_id'))$this->input->post('category_id');
// exit();
           ...
           // $this->form_validation->set_rules('category_id', 'category_id');
            ...
            $insert['admin_per'] = $this->input->post('admin_per');
            $insert['breaking'] = $this->input->post('breaking');
            $insert['show_on_menu'] = $this->input->post('show_on_menu');
            $insert['youtube_link'] = $this->input->post('youtube_link');
             $insert['server_image'] = $this->input->post('server_image');
           ...
           //  $featuredimgtmp=$_FILES['featured_img']['tmp_name'];
            ...
            if($insert['content_type'] == "Page" || "Services")
            {
                ...
                $insert_category['content_id'] = $result;
                foreach ($this->input->post('category_id') as $value) {
                     $insert_category['category_id'] = $value;
                     $this->crud->insert($insert_category,'multi_cat');
                }
               ...
               if(!empty($new_img)){
                   $insert['server_image']     = $new_img;
               }
               else{
                   $insert['server_image']     = $same_img;
               }
...
//                    exit;
        ...
        $category = $this->crud->get_where('multi_cat',array('content_id'=>$id));
        $cats=array();
        foreach ($category as $key => $value) {
           $cats[]=$value['category_id'];
        }
        $data['category_selected']=$cats;
    ...
    public function slider_content($content_id){
        if(strlen($content_id) <= 0){ redirect('news'); }
    ...
    public function special_content($content_id){
        if(strlen($content_id) <= 0){ redirect('news'); }
        $content_id = $content_id;
        $checkProduct = $this->crud->get_detail($content_id, "content_id", "igc_content");
        if(count($checkProduct) > 0){
            $update['updated'] = date('Y-m-d h:i:s');
            $update['special_content'] = "1";
    ...
    public function non_special_content($content_id){
        if(strlen($content_id) <= 0){ redirect('news'); }
        $content_id = $content_id;
        $checkProduct = $this->crud->get_detail($content_id, "content_id", "igc_content");
        if(count($checkProduct) > 0){
            $update['updated'] = date('Y-m-d h:i:s');
            $update['special_content'] = "0";
        ...
        if($this->input->post()){
            ...
            if(strlen($_FILES['image']['name']) > 0){
                ...
                if(in_array($extension,$allowed_type)){
                    ...
                    redirect('news/list_photos/'.$this->input->post('content_id')); 
                }
            }else{
                if($id > 0){
...
//code to add new tags in content
    ...
    public function send($content_id)
    {
        ...
        if(! empty($content_detail) && $content_detail['content_type'] == 'Article' && ! empty($subscribes))
        {
            ...
            $server_url = $site_settings['site_url'].'/index.php/news/detail/'.$content_detail['content_url'];
