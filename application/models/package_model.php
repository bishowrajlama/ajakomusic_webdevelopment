...
class Package_model extends CI_Model
{
    ...
    public function get_package_am($slug){
...
//        $string = rtrim($string,",");
        ...
        $query2 = $this->db->query("SELECT * FROM `igc_package` WHERE `package_url` = '$slug' ");
...
//    }
    ...
    public function get_related_packages($slug, $start_point = 0, $per_page = 0)
    {
        ...
        $query = $this->db->query("select p.package_id, p.featuredimg, p.tourcode, p.summary, p.summary, p.package_name,p.package_url,p.description, p.rating, p.brand_id ,pp.pprice,c.code from
...
//    }
    ...
    public function get_rel_cats($id, $start_point = 0, $per_page = 0){
        /*$detail = $this->crud->get_active_not_deleted_detail($slug, 'category_url', 'igc_package_category');
        $cat_id = $detail['category_id']*/
...
//        $string = rtrim($string,",");
        ...
        $query2 = $this->db->query("SELECT * FROM `igc_package` WHERE `category_id` = $id AND `delete_status` ='0' limit
		{$start_point},{$per_page}");
        $result2 = $query2->result_array();
